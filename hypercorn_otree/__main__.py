...
def main(sys_args: Optional[List[str]] = None) -> None:
    ...
    parser.add_argument(
        "--ping-interval",
        help="""Seconds to wait after last websocket client response before sending a Ping.
        (default: 20; 0 to disable Ping)""",
        default=sentinel,
        type=int,
    )
    parser.add_argument(
        "--ping-timeout",
        help="""Seconds to wait after before closing the websocket if no response from client.
        (default: 30; 0 to disable timeout)""",
        default=sentinel,
        type=int,
    )
    ...
    if args.ping_interval is not sentinel:
        config.ws_ping_interval = args.ping_interval
    if args.ping_timeout is not sentinel:
        config.ws_ping_timeout = args.ping_timeout
