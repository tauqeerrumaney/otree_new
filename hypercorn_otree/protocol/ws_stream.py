...
class WSStream:
    def __init__(
        ...
        self.ping_task: Union[asyncio.Task, None] = None
        ...
        if isinstance(event, Request):
            ...
            if not self.handshake.is_valid():
                ...
                if self.ping_interval:
                    self.ping_task = await self.async_ctx.start_task(self._ping_pong)
        elif isinstance(event, (Body, Data)):
            if self.ping_task is not None:
                self.ping_task.cancel()
            self.connection.receive_data(event.data)
            await self._handle_events()
            if self.ping_interval and not self.closed:
                self.ping_task = await self.async_ctx.start_task(self._ping_pong)
    ...
    async def _ping_pong(self):
        while True:
            await self.async_ctx.sleep(self.ping_interval)
            await self._send_wsproto_event(Ping())
            if self.ping_timeout:
                await self.async_ctx.sleep(self.ping_timeout)
                await self.async_ctx.start_task(self._cancel_due_to_timeout)
                break

    async def _cancel_due_to_timeout(self):
        await self._send_wsproto_event(CloseConnection(code=CloseReason.ABNORMAL_CLOSURE))
        await self.send(StreamClosed(stream_id=self.stream_id))
